...
def _legacy_scoring_helpers():
    try:
        from shared.extractor import check_exact_match, extract_ground_truth, normalize_answer_str
...
def evaluate_ir_variant(
    ...
    for item in dataset:
        ...
        print(f"[{variant} {new_count}/{len(dataset)}] Sample {new_count} (Level {result['level']}): {status_tag} | Latency: {latency:.1f}s | Tokens: {result['generated_tokens']} | Verif: {result['verification_status']}")
...
evaluate_symplan = evaluate_ir_variant
evaluate_ir_full = evaluate_ir_variant
