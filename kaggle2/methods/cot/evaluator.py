"""
Evaluator for Chain-of-Thought (CoT) reasoning.
"""
...
from .prompts import build_cot_messages
...
def _load_checkpoint(checkpoint_file: Optional[str]) -> tuple[dict, set]:
    ...
    try:
        with open(checkpoint_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        results = data.get("results", {}).get("CoT", [])
...
def _save_checkpoint(checkpoint_file: Optional[str], results: List[Dict[str, Any]]) -> None:
    ...
    if os.path.exists(checkpoint_file):
        try:
            ...
            pass
    data.setdefault("results", {})["CoT"] = results
...
def evaluate_cot(
    ...
    """
    Danh gia phuong phap CoT (Chain-of-Thought).
    """
    existing_data, completed = _load_checkpoint(checkpoint_file)
    results = existing_data.get("results", {}).get("CoT", [])

    if completed:
        print(f"[INFO] Tiep tuc CoT: da hoan thanh {len(completed)}/{len(dataset)} mau.")

    print(f"\n==================== Bat dau danh gia Phuong phap: CoT ====================")

    new_count = 0
    for item in tqdm(dataset, desc="Danh gia CoT"):
