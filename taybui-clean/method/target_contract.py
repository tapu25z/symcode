...
def _parse_labeled_planner(text: str) -> dict[str, Any] | None:
    ...
    has_label = re.search(
        r"^\s*#\s*(?:Subject|Target|Given|Variables|Relations|Constraints|Step\s+\d+|Answer\s+type)\s*:",
        text,
    ...
    def semicolon_list(label: str) -> list[str]:
        raw = field(label)
        return [part.strip() for part in raw.split(";") if part.strip()] or ([raw] if raw else [])

    variables = semicolon_list("Variables")
    relations = semicolon_list("Relations")
    constraints = semicolon_list("Constraints")
    given = semicolon_list("Given")
    if not given:
        given = variables + relations
    steps = [
        match.group(1).strip()
