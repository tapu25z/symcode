...
def evaluate_symplanner(
    ...
    - Turn 1: Extract trạng thái toán học, không giải.
    - Turn 2: Lập plan từ problem + extract, chưa in/tính đáp án cuối.
    ...
    for item in tqdm(dataset, desc="Danh gia SymPlanner", disable=not verbose):
        ...
        if exec_res["status"] == "success" and candidate_ans is not None:
            verif_status, verif_feedback = verify_candidate_answer(
                question, candidate_ans, extracted_code, exec_res.get("stdout"), symplanner_context
            )
        else:
            verif_status = "fail"
