...
def normalize_answer_str(ans: Optional[str]) -> str:
    ...
    Xử lý phân số, căn thức, góc độ, tọa độ, nhân ẩn, và loại bỏ wrapper LaTeX/đơn vị đo.
    """
    if ans is None:
        return ""
    ...
    # Xử lý căn thức: \sqrt[n]{x}, \sqrt{x}, và \sqrt x (không ngoặc nhọn)
    ...
    # Chèn dấu nhân ẩn cho căn thức và pi (ví dụ 3sqrt(13) -> 3*sqrt(13), 3pi -> 3*pi)
    ...
    s = re.sub(r"(?<=\d)(?:hours?|hrs?|minutes?|mins?|seconds?|secs?|miles?|mph|meters?|cm\^2|cm|km|dollars?|cents?|eggs?|days?|weeks?|years?|units?|sq\s*units?)$", "", s, flags=re.IGNORECASE)
...
def check_exact_match(pred: Optional[str], gt: str) -> bool:
    ...
    # So sánh đại số SymPy với hỗ trợ parse_expr nhân ẩn (implicit multiplication)
    try:
        import sympy
        ...
        try:
            ...
            p_sym = parse_expr(norm_pred, transformations=transformations)
