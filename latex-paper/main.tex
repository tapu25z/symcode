...
Furthermore, as illustrated in Fig.~\ref{fig:chart}, SymPlan markedly improves the Execution Success Rate across all models, increasing the cross-model average from $83.83\%$ to $89.90\%$ ($+6.07\%$). Notably, on Llama-3.1-8B and Qwen3-4B, SymPlan elevates ESR by $+8.00\%$ and $+8.60\%$, respectively. This demonstrates that pre-specifying output contracts and enforcing exact SymPy code rules substantially reduces syntax errors and undefined variable exceptions before code reaches the interpreter, lowering execution failures by $37.5\%$ across all models.

Regarding computational efficiency, SymPlan maintains an average token cost of $392.9$ tokens per problem across all models (accounting for the fused plan of $\approx 85$ tokens and code synthesis of $\approx 305$ tokens), which remains comparable to monolithic SymCode ($320.6$ tokens) while achieving a $+4.20\%$ accuracy boost, and noticeably more compact than natural-language CoT ($423.7$ tokens).
...
\textbf{2. Mitigation of Format and Type Mismatches (-23.5\%).} Format mismatches (where models output unboxed text, template artifacts, or raw solution sets when an integer count is requested) dropped from 515 instances in SymCode to 394 in SymPlan, representing a $23.5\%$ reduction. Enforcing the explicit type contract $\Omega \in \{\texttt{number}, \texttt{text}, \texttt{tuple}, \texttt{set}, \texttt{symbolic}\}$ during the fused planning turn directly prevents models from confusing cardinality with element enumeration.

\textbf{3. Semantic Misformulation and Level-5 Boundaries.} While SymPlan effectively suppresses program crashes and contract violations, semantic misformulation (cleanly executing scripts that compute an incorrect mathematical value) remains a persistent challenge for both methods ($\approx 32.3\%$). This confirms that on advanced Level-5 competition problems, the primary hurdle shifts from code execution to deep conceptual deduction. In these extreme settings, combining symbolic execution with formal interactive proof assistants (e.g., Lean) offers a promising future avenue.
\FloatBarrier
