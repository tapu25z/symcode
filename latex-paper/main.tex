...
These cases explain why CoT can outperform monolithic SymCode on compact models despite SymCode's high ESR. Traceback-guided repair is effective for syntax, API, and type failures, but it cannot reliably detect a runnable program that encodes the wrong target, transformation, or constraint. The reconstructed SymPlan examples illustrate the core mechanism: target contracts stabilize the mathematical state, planning selects a valid transformation sequence, and symbolic execution is strictly anchored to these explicit contracts.
