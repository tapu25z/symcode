...
\end{align}
where $\tau$ is the explicit target specification, $\Omega$ denotes the output type contract, $\mathcal{P}$ is the ordered symbolic plan, and $\mathcal{H}$ is the history of execution attempts. By freezing the problem contract before code synthesis and enforcing monotonic selection across debugging iterations, SymPlan eliminates semantic drift without requiring iterative human guidance.
...
\begin{itemize}
    ...
    \item \textbf{Output Type Contract ($\Omega$):} A strict categorical type $\Omega \in \{\texttt{number}, \texttt{text}, \texttt{tuple}, \texttt{set}, \texttt{symbolic}\}$. For instance, if the prompt asks ``how many'', $\Omega$ is mandated as \texttt{number} representing cardinality ($\operatorname{card}(S)$), strictly prohibiting the emission of the raw solution set $S$.
...
\textbf{Technical Advantages.} Compressing target extraction and planning into a single bounded forward pass cuts token latency by over $45\%$ compared to multi-turn prompting while maintaining strict grounding. Furthermore, explicitly declaring $\Omega$ establishes a rigid invariant that guides subsequent code synthesis and downstream parsing.
...
\textbf{Module Design.} The code generator $\operatorname{CodeGen}_{\mathcal{M}}$ consumes problem $q$ conditioned on the frozen contract $(\tau, \Omega, \mathcal{P})$ and generates a self-contained Python script using SymPy~\cite{meurer2017sympy}. The generation is governed by deterministic rules enforced directly through few-shot demonstration contracts:
\begin{enumerate}
    \item \textit{Exact Symbolic Arithmetic:} All rational values are constructed via \texttt{sp.Rational(p, q)} strictly for integers $p, q \in \mathbb{Z}$, or symbolic division \texttt{sp.S(p) / q} for radical expressions. Floating-point conversions via \texttt{.evalf()} are banned unless explicit decimal precision is queried.
    \item \textit{Domain Filtering and Validation:} When solving polynomial or transcendental systems via \texttt{sp.solve}, candidate roots are explicitly filtered against problem constraints (e.g., $x \in \mathbb{R}^+$, integer modular domains, non-zero denominators).
...
# 3. Emit strictly formatted answer conforming to \Omega
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Multi-Tier Fallback Strategy:} If $r^*$ yields an unparseable or empty prediction, SymPlan executes a deterministic fallback cascade: (i) reverse-scanning raw model responses for embedded boxed expressions, (ii) extracting regex candidates from plan notes, and (iii) enforcing type compliance via $\operatorname{FormatContract}(\hat{y}, \Omega)$.
\end{itemize}

\textbf{Technical Advantages.} The anti-regression ranking guarantees monotonically non-decreasing solution quality across repair iterations, completely neutralizing the silent regression trap common in smaller language models.
...
We evaluate all methods on the complete \textbf{MATH-500} benchmark~\cite{lightman2024verify,hendrycks2021math}, a rigorous, standardized 500-problem evaluation suite sampled from the challenging competition-grade MATH benchmark. MATH-500 spans seven core mathematical subjects (\textit{Algebra, Intermediate Algebra, Number Theory, Counting \& Probability, Geometry, Precalculus, Prealgebra}) and five graduated difficulty levels (Level 1 through Level 5).

\subsubsection{Model and Execution Configuration}

To test scalability and architectural suitability across compact model families and parameter scales, we evaluate six representative open-weight language models:
\begin{itemize}
...
\end{itemize}
All models are evaluated under 4-bit NormalFloat4 (NF4) quantization using \texttt{bitsandbytes}~\cite{dettmers2023qlora} with Scaled Dot-Product Attention (SDPA). Across all methods, decoding is fully deterministic via greedy search: \texttt{temperature} $= 0.0$, \texttt{top\_p} $= 1.0$, and a generation ceiling of \texttt{max\_new\_tokens} $= 1024$. In-sandbox program execution is strictly enforced with a timeout threshold $\tau = 15$ seconds and a maximum retry budget $K = 2$.
...
\addplot[fill=accentblue!70, draw=accentblue!90] coordinates {
...
\end{axis}
\end{tikzpicture}
\caption{Execution Success Rate (ESR \%) comparison between SymCode and SymPlan across all six compact models on MATH-500.}
\label{fig:chart}
\end{figure}
...
\begin{table*}[t]
\centering
\caption{Subject-wise accuracy (\%) across all seven mathematical disciplines on MATH-500. The SymPlan column is highlighted.}
\label{tab:subjects}
\begin{minipage}{0.49\textwidth}
\centering
\textbf{Qwen2.5-Coder-7B-Instruct}\\[1mm]
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccc>{\columncolor{ourgreen}}c}
\toprule
\rowcolor{headergray}
\textbf{Subject} & \textbf{Count} & \textbf{Direct} & \textbf{CoT} & \textbf{SymCode} & \textbf{SymPlan} \\
\midrule
Algebra & 124 & 25.00 & 51.61 & 66.94 & \textbf{76.61} \\
Counting \& Probability & 38 & 21.05 & 39.47 & 52.63 & \textbf{57.89} \\
Geometry & 41 & 19.51 & 36.59 & 31.71 & \textbf{34.15} \\
Intermediate Algebra & 97 & 15.46 & 16.49 & \textbf{51.55} & 48.45 \\
Number Theory & 62 & 17.74 & 35.48 & 66.13 & \textbf{70.97} \\
Prealgebra & 82 & 32.93 & 62.20 & 69.51 & \textbf{75.61} \\
Precalculus & 56 & 17.86 & 14.29 & 37.50 & \textbf{41.07} \\
\bottomrule
\end{tabular}}
\end{minipage}
\hfill
...
\textbf{Subject} & \textbf{Count} & \textbf{Direct} & \textbf{CoT} & \textbf{SymCode} & \textbf{SymPlan} \\
\midrule
Algebra & 124 & 54.03 & 64.52 & 71.77 & \textbf{75.81} \\
Counting \& Probability & 38 & 36.84 & 50.00 & 44.74 & \textbf{57.89} \\
Geometry & 41 & 21.95 & 39.02 & 46.34 & \textbf{51.22} \\
Intermediate Algebra & 97 & 10.31 & 15.46 & 50.52 & \textbf{51.55} \\
Number Theory & 62 & 37.10 & 40.32 & 62.90 & \textbf{72.58} \\
Prealgebra & 82 & 62.20 & 73.17 & 73.17 & \textbf{73.17} \\
Precalculus & 56 & 12.50 & 17.86 & 46.43 & \textbf{48.21} \\
\bottomrule
\end{tabular}}
\end{minipage}
\end{table*}
...
\end{axis}
\end{tikzpicture}
\caption{Accuracy gains over SymCode~\cite{nezhad2025symcode} (percentage points) in the component ablation across all six models on MATH-500.}
\label{fig:ablation}
\end{figure}
...
\subsection{Error Analysis and Qualitative Diagnostics}
\label{sec:error_analysis}

A central empirical observation across all six evaluated backbones is the significant divergence between Execution Success Rate (ESR) and mathematical correctness. As detailed in Table~\ref{tab:mainresults} and Fig.~\ref{fig:chart}, monolithic SymCode achieves an average ESR of $83.83\%$, yet delivers an average accuracy of only $50.37\%$. This indicates that over $33\%$ of the generated programs execute cleanly to termination without raising exceptions or tracebacks, yet still output mathematically incorrect answers. For compact language models, program execution alone acts as a certificate of syntactic validity rather than semantic truth.

Through detailed examination of the failed execution traces across our benchmark logs, we identify three predominant failure modes in monolithic generation that SymPlan's architecture directly mitigates:

\textbf{1. Domain Constraint Omission and Extraneous Roots.} In multi-step algebraic problems (such as radical simplification or geometric coordinate derivations), compact models generating unguided code frequently omit implicit domain restrictions ($x \ge 0$, real-valued bounds, or non-zero denominators). When invoking \texttt{sp.solve}, the solver returns multiple symbolic roots (e.g., negative and positive branches), and unconstrained models routinely select the first returned element without verification. SymPlan's contract-guided generation explicitly carries extracted domain invariants into the Python script, forcing candidate filtering before formatting the final result.

\textbf{2. Target Type and Cardinality Mismatches.} A frequent source of evaluation failure in monolithic code synthesis involves target contract misalignment. In combinatorial counting problems asking ``how many'' configurations exist, unconstrained models often return the raw list or set of candidate instances rather than the integer cardinality $\operatorname{card}(S)$. By establishing the strict categorical contract $\Omega \in \{\texttt{number}, \texttt{text}, \texttt{tuple}, \texttt{set}, \texttt{symbolic}\}$ during the fused planning turn, SymPlan eliminates this discrepancy by strictly enforcing cardinality computation (\texttt{len(solutions)}) prior to boxed answer emission.

\textbf{3. Silent Semantic Regressions during Self-Repair.} In standard traceback-guided debugging loops, when a model attempts to fix an execution error, the revision prompt often causes compact models to abandon their initial algebraic plan in favor of an oversimplified, trivialized script (e.g., hardcoding a constant or printing intermediate variables). While such scripts execute with zero runtime errors (status: \texttt{success}), they silently degrade a previously viable solution path. SymPlan's anti-regression ranking metric $\operatorname{Rank}(r)$ neutralizes this vulnerability by evaluating the entire episodic attempt history $\mathcal{H}$, guaranteeing that speculative later revisions cannot overwrite valid earlier attempts.

\textbf{Failure Boundaries and Level-5 Limitations.} On the most challenging Level-5 problems, where mathematical solutions require deep non-constructive deductions, complex geometric auxiliary constructions, or non-standard number-theoretic identities, code-based computation alone reaches intrinsic boundaries. In these settings, compact language models struggle both in formulating symbolic plans and in structuring procedural searches. Integrating planning-guided neurosymbolic frameworks with interactive formal theorem provers (such as Lean) represents a vital direction for extending reasoning capability on advanced competition mathematics.

\FloatBarrier

\section{Conclusion}

In this paper, we presented SymPlan, an inspectable neurosymbolic framework tailored for mathematical reasoning in compact open-weight language models. By decomposing monolithic symbolic generation into a fused target-planning contract, contract-guided SymPy code synthesis, and targeted debugging with anti-regression selection, SymPlan directly resolves semantic misformulation, domain omission, and silent repair regressions. Comprehensive evaluations across six diverse models (spanning 3B to 8B parameters) on the full MATH-500 benchmark demonstrate uniform accuracy improvements of $+3.60\%$ to $+5.20\%$ over SymCode and up to $+23.20\%$ over Chain-of-Thought, alongside a $+6.07\%$ increase in execution success rate, all without parameter fine-tuning.
