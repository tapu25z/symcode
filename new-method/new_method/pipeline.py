...
from .prompts import codegen_prompt, extraction_prompt, ir_repair_prompt, repair_prompt
...
class SymPlannerIRPipeline:
    def __init__(self, llm_call: Callable[[list[dict[str, str]]], str], execute_code: Callable[[str], Mapping[str, Any]], max_repairs: int = 2, max_ir_repairs: int = 1):
    ...
    def run(self, question: str) -> dict[str, Any]:
        ...
        ir_repairs = []
        for attempt in range(self.max_ir_repairs):
            ...
            schema_errors = collect_schema_errors(raw_ir, ir)
        if schema_errors:
            return {"status": "invalid_ir", "question": question, "ir": ir, "payload": None, "code": None, "schema_errors": schema_errors, "ir_repairs": ir_repairs, "attempts": [], "final": {"status": "invalid_ir", "errors": schema_errors}}
        normalized = normalize_problem_ir(ir)
        normalization_errors = validate_normalized_ir(normalized)
        if normalization_errors:
            return {"status": "invalid_ir", "question": question, "ir": normalized, "payload": None, "code": None, "schema_errors": normalization_errors, "ir_repairs": ir_repairs, "attempts": [], "final": {"status": "invalid_ir", "errors": normalization_errors}}
        ...
        return {"status": final_status, "question": question, "ir": normalized, "payload": payload, "code": code, "schema_errors": schema_errors, "ir_repairs": ir_repairs, "attempts": attempts, "final": attempts[-1] if attempts else None}
